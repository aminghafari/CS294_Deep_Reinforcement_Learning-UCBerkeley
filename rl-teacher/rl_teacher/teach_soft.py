...
####### Extra variables
n_generated_pairs = 50
n_pool = 20
good_segs_pool = deque(maxlen = n_pool)
bad_segs_pool  = deque(maxlen = n_pool)
...
#        return q_value[0]
    ...
    def path_callback(self, path):
        ...
        # Generate a set of pairs an choose the one with the highest std between reward functions
        # random_segment = basic_segments_from_rand_rollout_1(self.env_id, make_with_torque_removed, n_desired_segments = 1, clip_length_in_seconds=CLIP_LENGTH)[0]
        if len(self.comparison_collector) < int(self.label_schedule.n_desired_labels):
            ...
            self.comparison_collector.add_segment_pair(chosen_pair['segment1'],chosen_pair['segment2'])
